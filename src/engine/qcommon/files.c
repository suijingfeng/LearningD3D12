...
typedef struct {
	char			pakFilename[MAX_OSPATH];	// c:\quake3\baseoa\pak0.pk3
	char			pakBasename[MAX_OSPATH];	// pak0
	char			pakGamename[MAX_OSPATH];	// baseoa
...
typedef struct {
	...
	char		gamedir[MAX_OSPATH];	// baseoa
} directory_t;
...
A mod directory is a peer to baseoa with a pk3 in it
...
int	FS_GetModList( char *listbuf, int bufsize ) {
  ...
  *listbuf = 0;
  ...
  for ( i = 0 ; i < nPotential ; i++ ) {
    ...
    // we drop "baseq3" "." and ".."
    if (Q_stricmp(name, "baseoa") && Q_stricmpn(name, ".", 1)) {
...
qboolean FS_ComparePaks( char *neededpaks, int len, qboolean dlstring ) {
	...
	*neededpaks = 0;
	...
	for ( i = 0 ; i < fs_numServerReferencedPaks ; i++ ) {
		...
		if ( FS_idPak(fs_serverReferencedPakNames[i], "baseoa") || FS_idPak(fs_serverReferencedPakNames[i], "missionpack") ) {
			continue;
		}
