...
static VkSwapchainKHR create_swapchain(VkPhysicalDevice physical_device, VkDevice device, VkSurfaceKHR surface, VkSurfaceFormatKHR surface_format) {
	...
	uint32_t nPM, i;
	VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &nPM, nullptr));
	VkPresentModeKHR *pPresentModes =
		(VkPresentModeKHR *)malloc(nPM * sizeof(VkPresentModeKHR));
	VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &nPM, pPresentModes));
...
static const char* extension_names_supported[64] = { 0 };
static unsigned int enabled_extension_count = 0;
...
void vk_create_instance(void)
{
	...
	if (instance_extension_count > 0)
	{
		...
		VkResult err = vkEnumerateInstanceExtensionProperties(NULL, &instance_extension_count, instance_extensions);
		if (err < 0)
	...
	desc.enabledExtensionCount = enabled_extension_count;
	desc.ppEnabledExtensionNames = extension_names_supported;
	...
	VkResult e = vkCreateInstance(&desc, NULL, &vk.instance);
	if (!e)
	{
...
void selectPhysicalDevice(void)
{
	...
	VkResult res = vkEnumeratePhysicalDevices(vk.instance, &gpu_count, pPhyDev);
	if (res < 0)
...
void selectSurfaceFormat(void)
{
	uint32_t nSurfmt;

	// Get the list of VkFormat's that are supported

	VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(vk.physical_device, vk.surface, &nSurfmt, NULL));
	assert(nSurfmt > 0);

	VkSurfaceFormatKHR *pSurfFmts =
		(VkSurfaceFormatKHR *)malloc(nSurfmt * sizeof(VkSurfaceFormatKHR));

	VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(vk.physical_device, vk.surface, &nSurfmt, pSurfFmts));
...
void selectQueueFamily(void)
{

	uint32_t nSurfmt;
	uint32_t i;

	vkGetPhysicalDeviceQueueFamilyProperties(vk.physical_device, &nSurfmt, NULL);
	...
	vkGetPhysicalDeviceQueueFamilyProperties(vk.physical_device, &nSurfmt, pQueueFamilies);
...
void lookforDeviceExtensions(void)
{
	// create VkDevice
...
void vk_initialize(void)
{
	...
	//
	{
		...
		for (uint32_t i = 0; i < vk.swapchain_image_count; i++) {
			...
			desc.subresourceRange.baseArrayLayer = 0;
			desc.subresourceRange.layerCount = 1;
			VK_CHECK(vkCreateImageView(vk.device, &desc, NULL, &vk.swapchain_image_views[i]));
		}
	}
	...
	//
	{
		...
		VK_CHECK(vkCreateSemaphore(vk.device, &desc, NULL, &vk.image_acquired));
		VK_CHECK(vkCreateSemaphore(vk.device, &desc, NULL, &vk.rendering_finished));

		VkFenceCreateInfo fence_desc;
		fence_desc.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
		fence_desc.pNext = NULL;
		fence_desc.flags = VK_FENCE_CREATE_SIGNALED_BIT;
		VK_CHECK(vkCreateFence(vk.device, &fence_desc, NULL, &vk.rendering_finished_fence));
	}
	...
	// 
	{
		...
		// create depth image
		{
			...
			desc.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
			VK_CHECK(vkCreateImage(vk.device, &desc, NULL, &vk.depth_image));
		}
		...
		// allocate depth image memory
		{
			...
			VK_CHECK(vkAllocateMemory(vk.device, &alloc_info, NULL, &vk.depth_image_memory));
			VK_CHECK(vkBindImageMemory(vk.device, vk.depth_image, vk.depth_image_memory, 0));
		}
		...
		VkCommandBuffer pCB;
		VK_CHECK(vkAllocateCommandBuffers(vk.device, &alloc_info, &pCB));
		...
		VK_CHECK(vkBeginCommandBuffer(pCB, &begin_info));
		// recorder(command_buffer);
		{
		...
		VK_CHECK(vkEndCommandBuffer(pCB));
		...
		VkResult res = vkQueueSubmit(vk.queue, 1, &submit_info, VK_NULL_HANDLE);
		if (res < 0)
			ri.Error(ERR_FATAL, "vk_initialize: error code %d returned by qvkQueueSubmit\n", res);

		VK_CHECK(vkQueueWaitIdle(vk.queue));
		vkFreeCommandBuffers(vk.device, vk.command_pool, 1, &pCB);
	...
	//
	{
		...
		for (uint32_t i = 0; i < vk.swapchain_image_count; i++) {
			attachments[0] = vk.swapchain_image_views[i]; // set color attachment
			VK_CHECK(vkCreateFramebuffer(vk.device, &desc, NULL, &vk.framebuffers[i]));
		}
	}
	...
	//
	{
		...
		VK_CHECK(vkCreateDescriptorSetLayout(vk.device, &desc, NULL, &vk.set_layout));
	}
	...
	//
	{
		...
		desc.size = VERTEX_BUFFER_SIZE;
		desc.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
		VK_CHECK(vkCreateBuffer(vk.device, &desc, NULL, &vk.vertex_buffer));

		desc.size = INDEX_BUFFER_SIZE;
		desc.usage = VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
		VK_CHECK(vkCreateBuffer(vk.device, &desc, NULL, &vk.index_buffer));
		...
		alloc_info.allocationSize = index_buffer_offset + ib_memory_requirements.size;
		alloc_info.memoryTypeIndex = memory_type;
		VK_CHECK(vkAllocateMemory(vk.device, &alloc_info, NULL, &vk.geometry_buffer_memory));
	...
	//
	{
		// skybox
		{
			...
			def.mirror = qfalse;
			def.line_primitives = false;
			def.shadow_phase = Vk_Shadow_Phase::disabled;
...
void vk_upload_image_data(VkImage image, int width, int height,
	bool mipmap, const uint8_t* pixels, int bytes_per_pixel)
{
	VkBufferImageCopy regions[16];
	int num_regions = 0;
	...
	//// recorder = [&image, &num_regions, &regions](VkCommandBuffer command_buffer)
	{
		...
		vkCmdCopyBufferToImage(cb, vk_world.staging_buffer, image,
...
void vk_read_pixels(byte* buffer)
{
	...
	alloc_info.memoryTypeIndex = find_memory_type(vk.physical_device, memory_requirements.memoryTypeBits,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	VkDeviceMemory memory;
	VK_CHECK(vkAllocateMemory(vk.device, &alloc_info, NULL, &memory));
	VK_CHECK(vkBindImageMemory(vk.device, image, memory, 0));
